...
class SocialOpenGraphConvertUrlToEmbedFilter extends ConvertUrlToEmbedFilter implements ContainerFactoryPluginInterface {
  ...
  public static function convertUrls($text, $url_prefix = '') {
    if (\Drupal::config('social_open_graph.settings')->get('settings')) {
        ...
        if ($chunk_type == 'text') {
          ...
          if ($open_tag == '') {
            ...
            $chunks[$i] = preg_replace_callback(
              ...
              function ($match) {
                try {
                  $info = \Drupal::service('social_open_graph')->getUrlInfo(Html::decodeEntities($match[1]));
                  if ($info) {
                  ...
                  watchdog_exception('social_open_graph', $e);
                  return $match[1];
                }
              },
              $chunks[$i]
